...
public class EntityNeedle extends Entity implements IProjectile, IEntityAdditionalSpawnData {
    ...
    public void onUpdate() {
        ...
        if (hit != null && !worldObj.isRemote) {
            if (MFRRegistry.getNeedleAmmoTypes()
                .containsKey(_ammoSource.getItem())) {
                if (hit.entityHit != null) {
                    MFRRegistry.getNeedleAmmoTypes()
                        .get(_ammoSource.getItem())
                        .onHitEntity(_ammoSource, owner, hit.entityHit, distance);
                } else {
                    MFRRegistry.getNeedleAmmoTypes()
                        .get(_ammoSource.getItem())
                        .onHitBlock(
        ...
        for (rotationPitch = (float) (Math.atan2(motionY, speed) * 180.0D / Math.PI); rotationPitch - prevRotationPitch
            < -180.0F;) {
        ...
        if (isInWater()) {
            ...
            for (int i = 0; i < 4; ++i) {
                worldObj.spawnParticle(
                    "bubble",
                    posX - motionX * particleOffset,
                    posY - motionY * particleOffset,
                    posZ - motionZ * particleOffset,
                    motionX,
                    motionY,
                    motionZ);
            }
