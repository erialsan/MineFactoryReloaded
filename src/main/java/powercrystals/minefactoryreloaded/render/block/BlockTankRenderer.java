...
public class BlockTankRenderer implements ISimpleBlockRenderingHandler, IItemRenderer {
    ...
    private void renderTank(Block block, ItemStack stack, RenderBlocks renderer) {
        ...
        IIcon iconFluid = renderer.getIconSafe(
            fluid != null ? fluid.getFluid()
                .getIcon(fluid) : renderer.getBlockIconFromSideAndMetadata(block, 6, 1));
        IIcon iconFluidTop = renderer
            .getIconSafe(fluid != null ? iconFluid : renderer.getBlockIconFromSideAndMetadata(block, 7, 1));
    ...
    public boolean renderWorldBlock(IBlockAccess blockAccess, int x, int y, int z, Block block, int modelId,
        ...
        boolean[] renderSide = { render || y <= 0 || block.shouldSideBeRendered(blockAccess, x, y - 1, z, 0),
