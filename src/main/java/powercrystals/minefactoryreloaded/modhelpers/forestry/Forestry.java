...
public class Forestry {
    ...
    private static void initForestry() {
        ...
        if (item != null) MFRRegistry.registerFertilizer(new FertilizerStandard(item, 0));
        else MineFactoryReloadedCore.log()
            .error("Forestry fertilizer null!");

        item = findItem(name, "fertilizerBio");
        if (item != null) MFRRegistry.registerFertilizer(new FertilizerStandard(item, 0));
        else MineFactoryReloadedCore.log()
            .error("Forestry compost null!");

        item = findItem(name, "peat");
        if (item != null) MFRRegistry.registerSludgeDrop(10, new ItemStack(item));
        else MineFactoryReloadedCore.log()
            .error("Forestry peat null!");

        item = findItem(name, "ash");
        if (item != null) MFRRegistry.registerSludgeDrop(1, new ItemStack(item));
        else MineFactoryReloadedCore.log()
            .error("Forestry ash null!");

        item = findItem(name, "decayingWheat");
        if (item != null) MFRRegistry.registerSludgeDrop(20, new ItemStack(item));
        else MineFactoryReloadedCore.log()
            .error("Forestry wheat null!");
        ...
        if (item != null && block != null) {
            ...
            .error("Forestry sapling/block null!");
        ...
        if (block != null) {
            ...
            .error("Forestry bog earth null!");

        for (int i = 1; true; ++i) {
            block = findBlock(name, "log" + i);
            l: if (block == null) {
                if (i > 1) MineFactoryReloadedCore.log()
                    .debug("Forestry logs null at " + i + ".");
                else {
