...
public class WorldGenMassiveTree extends WorldGenerator {
    ...
    private float layerSize(int par1) {
        ...
        if (par1 < leafBases) return -1.618F;
        else {
    ...
    private void generateLeafNodeList() {
        ...
        while (var6 >= 0) {
            ...
            if (var8 > 0.0F) for (float var9 = 0.5f; var7 < var1; ++var7) {
            ...
            for (int zMod = 0; zMod <= size;) { // negative values handled below
                ...
                if (distSq > maxDistSq) {
                    ...
                    do {
                        ...
                        if (safeGrowth
                            ? (block.isAir(worldObj, x, y, z) || block.isLeaves(worldObj, x, y, z)
    ...
    private void placeBlockLine(int[] par1, int[] par2, Block par3, int meta) {
        ...
        for (byte i = 0; i < 3; ++i) {
            ...
            var4[i] = a;
            if (b > ((a = var4[var6]) ^ (t = a >> 31)) - t) var6 = i;
        }
        ...
        for (int i = -trunkSize; i <= trunkSize; i++) {
            ...
            for (int j = -trunkSize; j <= trunkSize; j++) {
                if ((j * j + i * i) * 4 < trunkSize * trunkSize * 5) {
                    ...
                    if (slopeTrunk) var6[1] = var2 + sinc2(lim * i, lim * j, height) - (rand.nextInt(3) - 1);
    ...
    private int checkBlockLine(int[] par1, int[] par2) {
        ...
        for (byte i = 0; i < 3; ++i) {
            int a = par2[i] - par1[i], b = ((t = a >> 31) ^ a) - t;
            var3[i] = a;
            if (b > ((a = var3[var5]) ^ (t = a >> 31)) - t) var5 = i;
        }
        ...
        if (var3[var5] == 0) return -1;
            ...
            for (; var14 != var15; var14 += var8) {
                ...
                if (safeGrowth
                    ? !(var16.isAir(worldObj, x, y, z) || var16.isReplaceable(worldObj, x, y, z)
    ...
    private boolean validTreeLocation() {
        ...
        if (!block.canSustainPlant(
            ...
            basePos[2],
            ForgeDirection.UP,
            MFRThings.rubberSaplingBlock)) return false;
        else {
                ...
                for (int i = -trunkSize; i <= trunkSize; i++) {
                    ...
                    for (int j = -trunkSize; j <= trunkSize; j++) {
                        if ((j * j + i * i) * 4 < trunkSize * trunkSize * 5) {
                            ...
                            if (slopeTrunk) var6[1] = var2 + sinc2(lim * i, lim * j, height);
    ...
    public WorldGenMassiveTree setTreeScale(float height, float width, float leaves) {
        ...
        if (minHeight > 30) leafDistanceLimit = 5;
        else leafDistanceLimit = minHeight / 8;
    ...
    public void setBlockAndNotifyAdequately(World world, int x, int y, int z, Block block, int meta) {
        ...
        if (subChunk == null) storage[y >> 4] = subChunk = new ExtendedBlockStorage(y & ~15, !world.provider.hasNoSky);

        x &= 15;
        z &= 15;
        if (subChunk.getBlockByExtId(x, y & 15, z)
            .hasTileEntity(subChunk.getExtBlockMetadata(x, y & 15, z))) chunk.removeTileEntity(x & 15, y, z & 15);
        y &= 15;
