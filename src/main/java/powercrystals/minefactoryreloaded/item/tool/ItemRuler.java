            ...
            if (mop == null || (mop.typeOfHit == MovingObjectType.ENTITY && mop.entityHit == null)) {
                ...
                if (!data.hasKey("ruler")) {
                    ...
                    player.addChatMessage(
                        new ChatComponentText("X: ").appendText(
                            StatCollector.translateToLocalFormatted("chat.info.mfr.ruler.distance", distX, distX + 1)));
                    player.addChatMessage(
                        new ChatComponentText("Y: ").appendText(
                            StatCollector.translateToLocalFormatted("chat.info.mfr.ruler.distance", distY, distY + 1)));
                    player.addChatMessage(
                        new ChatComponentText("Z: ").appendText(
                            StatCollector.translateToLocalFormatted("chat.info.mfr.ruler.distance", distZ, distZ + 1)));
                    player.addChatMessage(
                        new ChatComponentText("")
                            .appendText(StatCollector.translateToLocalFormatted("chat.info.mfr.ruler.total", distAll)));
                }
            }
        }
