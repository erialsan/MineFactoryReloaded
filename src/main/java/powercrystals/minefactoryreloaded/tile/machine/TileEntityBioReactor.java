...
public class TileEntityBioReactor extends TileEntityFactoryInventory implements ITankContainerBucketable {
    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote) {
            ...
            for (int i = 0; i < 9; i++) {
                ...
                if (plantables.containsKey(item.getItem()) && plantables.get(item.getItem())
                    .canBePlanted(item, true)) {
                    ...
                    if (_inventory[targetSlot] == null) {
                        ...
                        if (_inventory[i].stackSize <= 0) _inventory[i] = null;
                    }
                }
            }

            if (_rednetState != 0 || CoreUtils.isRedstonePowered(this)) return;

            int newBurn = getOutputValue();
            if (_burnTimeMax - _burnTime >= newBurn) {
                _burnTime += newBurn;
                for (int i = 9; i < 18; i++) if (_inventory[i] != null) decrStackSize(i, 1);
            }
    ...
    public void writeItemNBT(NBTTagCompound nbttagcompound) {
        ...
        if (_burnTime != 0) nbttagcompound.setInteger("burnTime", _burnTime);
    }
