...
public class TileEntityRedNetCable extends TileEntityBase implements INode, ITraceable, ICustomHitBox {
    ...
    private static boolean isClient = FMLCommonHandler.instance()
        .getSide() == Side.CLIENT;
    ...
    public TileEntityRedNetCable() {
        ...
        if (_connectionBlackList == null) {
            ...
            for (String s : MFRConfig.redNetConnectionBlacklist.getStringList()) {
                if (!StringUtils.isNullOrEmpty(s)) connectionBlackList.add(Block.getBlockFromName(s));
                else System.out
                    .println("Empty or invalid rednet blacklist entry found. Not adding to rednet blacklist.");
            }
            List<Integer> wl = Arrays.asList(
                23,
                25,
                27,
                28,
            ...
            while (i-- > 0) if (!wl.contains(i)) connectionBlackList.add(Block.getBlockById(i));
            _connectionBlackList = connectionBlackList;
        }
        if (isClient) __lastUpdates = new long[10];
    }
    ...
    private void markForRegen() {
        ...
        int c = 0;
        for (int i = 6; i-- > 0;) if (_connectionState[i].isAllSubnets) ++c;
        if (c > 1) _network.regenerate();
    }
        ...
        if (_network == null) {
            for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
                ...
                if (BlockPosition.blockExists(this, dir)) {
                    TileEntityRedNetCable pipe = BlockPosition
                        .getAdjacentTileEntity(this, dir, TileEntityRedNetCable.class);
                    if (pipe != null) {
    ...
    public void updateInternalTypes(IGridController grid) {
        ...
        dirty = false;
        for (ForgeDirection d : dirs) updateNearbyNode(d);
        isRSNode = false;
        for (int i = _connectionState.length; i-- > 0;) {
            ForgeDirection d = dirs[i];
            int x = xCoord + d.offsetX, y = yCoord + d.offsetY, z = zCoord + d.offsetZ;
            if (worldObj.blockExists(x, y, z) && !worldObj.getBlock(x, y, z)
                .equals(rednetCableBlock)) isRSNode |= _connectionState[i].isConnected;
        }
        if (lastNode != isRSNode) _network.addConduit(this);
        markChunkDirty();
    ...
    public void addTraceableCuboids(List<IndexedCuboid6> list, boolean forTrace, boolean hasTool) {
        ...
        for (int i = sides.length; i-- > 0;) {
            ...
            if (c.isConnected) {
                if (c.isPlate) o += 6;
                else if (c.isCable) if (c.isAllSubnets) {
                    k = 2 + 6 * 3 + i;
                    list.add((IndexedCuboid6) new IndexedCuboid6(hasTool ? k + 18 : 1, subSelection[k]).add(offset)); // cable
                                                                                                                      // part
                    continue;
                }
                k = 2 + 6 * 3 + i;
                ...
                if (forTrace & hasTool & _cableMode[6] != 1 && (c = getConnectionState(sides[i], false)).isConnected) { // cable-only
                    if (c.isAllSubnets & c.isCable) {
                        k += 6 * 3;
                        o += 6 * 6;
                    }
                    list.add((IndexedCuboid6) new IndexedCuboid6(o, subSelection[k]).add(offset)); // connection point
                                                                                                   // (raytrace)
                }
        }
        main.add(offset);
    ...
    public CustomHitBox getCustomHitBox(int hit, EntityPlayer player) {
        ...
        for (int i = 1, e = list.size(); i < e; ++i) {
            ...
            if (shouldRenderCustomHitBox(data, player)) {
                if (cube.min.y < 0) box.sideLength[0] = Math.max(box.sideLength[0], -cube.min.y);
                if (cube.min.z < 0) box.sideLength[2] = Math.max(box.sideLength[2], -cube.min.z);
                if (cube.min.x < 0) box.sideLength[4] = Math.max(box.sideLength[4], -cube.min.x);
                cube.sub(max);
                if (cube.max.y > 0) box.sideLength[1] = Math.max(box.sideLength[1], cube.max.y);
                if (cube.max.z > 0) box.sideLength[3] = Math.max(box.sideLength[3], cube.max.z);
                if (cube.max.x > 0) box.sideLength[5] = Math.max(box.sideLength[5], cube.max.x);
            } else switch (BlockRedNetCable._subSideMappings[data]) {
                default: // no-op
                    break;
                case 0:
                case 7:
                    if (cube.min.y < 0) box.sideLength[0] = Math.min(box.sideLength[0], -cube.max.y);
                    break;
                case 1:
                case 8:
                    cube.sub(max);
                    if (cube.max.y > 0) box.sideLength[1] = Math.min(box.sideLength[1], cube.min.y);
                    break;
                case 2:
                case 9:
                    if (cube.min.z < 0) box.sideLength[2] = Math.min(box.sideLength[2], -cube.max.z);
                    break;
                case 3:
                case 10:
                    cube.sub(max);
                    if (cube.max.z > 0) box.sideLength[3] = Math.min(box.sideLength[3], cube.min.z);
                    break;
                case 4:
                case 11:
                    if (cube.min.x < 0) box.sideLength[4] = Math.min(box.sideLength[4], -cube.max.x);
                    break;
                case 5:
                case 12:
                    cube.sub(max);
                    if (cube.max.x > 0) box.sideLength[5] = Math.min(box.sideLength[5], cube.min.x);
                    break;
            }
        }
        for (int i = box.sideLength.length; i-- > 0;) box.drawSide[i] = box.sideLength[i] > 0;
        return box;
    }
    ...
    public boolean canInterface(TileEntityRedNetCable with, ForgeDirection dir) {
        ...
        return (with._cableMode[dir.getOpposite()
            .ordinal()] & 1) != 0;
    }
    ...
    public void getTileInfo(List<IChatComponent> info, ForgeDirection side, EntityPlayer player, boolean debug) {
        ...
        if (!debug) {
            ...
            if (_network != null) {
                ...
                String o = "[";
                for (int i = 0; i < 15; ++i) o += _network.getPowerLevelOutput(i) + ",";
    ...
    public boolean toggleSide(int side) {
        ...
        if (side != 6 && _connectionState[side].isAllSubnets) {
            removeFromGrid();
            removed = true;
            RedstoneNetwork.HANDLER.addConduitForUpdate(this);
        }
        ...
        if (removed) {
            incorporateTiles();
            if (_network == null) setNetwork(new RedstoneNetwork(this));
        }
    ...
    public byte getMode(int side) {
        ...
        if (side == 6) return _cableMode[side];
    ...
    public void setMode(int side, byte mode) {
        ...
        if (side != 6) {
            ...
            for (ForgeDirection d : ForgeDirection.VALID_DIRECTIONS) updateNearbyNode(d);
        }
        if (mustUpdate) worldObj.markBlockForUpdate(xCoord, yCoord, zCoord);
    }
    ...
    protected RedNetConnectionType getConnectionState(ForgeDirection side, boolean decorative) {
        ...
        if (decorative & (_mode & 1) == 0) return RedNetConnectionType.None;
        { // eclipse freaks out with bit shifts.
            _mode >>>= 1;
        }
        if (!decorative) _mode = 1;
        if (_cableMode[6] == 1) _mode = 3;

        int x = xCoord + side.offsetX;
        int y = yCoord + side.offsetY;
        int z = zCoord + side.offsetZ;

        if (!worldObj.blockExists(x, y, z)) return RedNetConnectionType.None;
        ...
        if (b == rednetCableBlock) {
            if (!decorative
                || ((TileEntityRedNetCable) worldObj.getTileEntity(x, y, z)).canInterface(this, side.getOpposite()))
                return RedNetConnectionType.CableAll;
            else return RedNetConnectionType.None;
        }
        ...
        if (b instanceof IRedNetConnection) { // API node - let them figure it out
            ...
            if (_mode == 0 && type.isConnectionForced) return RedNetConnectionType.None;
            else if (!type.isDecorative) return type;
            node = true;
        ...
        if (_mode == 2) {
            if (b.isSideSolid(worldObj, x, y, z, side.getOpposite())) ret = RedNetConnectionType.ForcedCableSingle;
            else ret = RedNetConnectionType.ForcedPlateSingle;
        }
        ...
        else if (!node && (_connectionBlackList.contains(b) || b instanceof IRedNetDecorative)) {
    ...
    public void readFromNBT(NBTTagCompound tag) {
        ...
        if (_sideColors.length < 6) _sideColors = new int[6];
        ...
        switch (tag.getByte("v")) {
            case 0:
                if (_mode == 2) _mode = 3;
            case 1:
                _cableMode = new byte[] { _mode, _mode, _mode, _mode, _mode, _mode, _mode };
            case 2:
                _cableMode[6] = (byte) (_cableMode[6] == 3 ? 1 : 0);
            case 3:
                for (int i = 6; i-- > 0;) _cableMode[i] = (byte) ((_cableMode[i] << 1) | 1);
            case 4:
    ...
    public boolean isSolidOnSide(int side) {
        ...
        if (_cableMode[6] == 1) return false;
    ...
    public boolean onRender() {
        ...
        long gap = 0;
        while (i-- > 1) gap += a[(i + p + 1) % e] - a[(i + p) % e];
        __useTESR = gap <= (30 * e);
        return __useTESR;
    }
    ...
    public Packet getDescriptionPacket() {
        ...
        data.setInteger(
            "mode[0]",
            (_cableMode[0] & 0xFF) | ((_cableMode[1] & 0xFF) << 8)
                | ((_cableMode[2] & 0xFF) << 16)
                | ((_cableMode[3] & 0xFF) << 24));
        data.setInteger(
            "mode[1]",
            (_cableMode[4] & 0xFF) | ((_cableMode[5] & 0xFF) << 8) | ((_cableMode[6] & 0xFF) << 16));
        data.setInteger(
            "state[0]",
            _connectionState[0].ordinal() | (_connectionState[1].ordinal() << 4)
                | (_connectionState[2].ordinal() << 8)
                | (_connectionState[3].ordinal() << 12)
                | (_connectionState[4].ordinal() << 16)
                | (_connectionState[5].ordinal() << 20));
