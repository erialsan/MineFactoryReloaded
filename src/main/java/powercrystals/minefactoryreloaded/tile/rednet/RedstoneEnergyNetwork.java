...
public class RedstoneEnergyNetwork implements IGrid {
    ...
    public int getNodeShare(TileEntityRedNetEnergy cond) {
        ...
        if (size <= 1) return storage.getEnergyStored();
        int amt = 0;
    ...
    public boolean addConduit(TileEntityRedNetEnergy cond) {
        if (conduitSet.add(cond)) if (!conduitAdded(cond)) return false;
        if (cond.isNode) {
            if (nodeSet.add(cond)) {
                nodeAdded(cond);
            }
            ...
            for (int i = 6; i-- > 0;) {
                ...
                if (world.blockExists(bp.x, bp.y, bp.z)) {
                    ...
                    if (te instanceof TileEntityRedNetEnergy) {
                        ...
                        if (main.canInterface(ter, dir[i ^ 1]) && checked.add(ter)) toCheck.add(ter);
                    }
                }
            }
            oldSet.remove(main);
    ...
    public void destroyGrid() {
        master = null;
        regenerating = true;
        for (TileEntityRedNetEnergy curCond : nodeSet) destroyNode(curCond);
        for (TileEntityRedNetEnergy curCond : conduitSet) destroyConduit(curCond);
    ...
    public void doGridPreUpdate() {
        ...
        if (tank.getEnergyStored() >= tank.getMaxEnergyStored()) return;
        ForgeDirection[] directions = ForgeDirection.VALID_DIRECTIONS;

        for (TileEntityRedNetEnergy cond : nodeSet) for (int i = 6; i-- > 0;) cond.extract(directions[i], tank);
    }
    ...
    public void doGridUpdate() {
        ...
        if (sideDistribute > 0) for (TileEntityRedNetEnergy cond : nodeSet) if (cond != master) {
            int e = 0;
            for (int i = 6; i-- > 0;) e += cond.transfer(directions[i], sideDistribute);
        ...
        if (sideDistribute > 0) {
            int e = 0;
            for (int i = 6; i-- > 0;) e += master.transfer(directions[i], sideDistribute);
            if (e > 0) storage.modifyEnergyStored(-e);
        } else if (toDistribute > 0) {
            int e = 0;
            for (int i = 6; i-- > 0 && e < toDistribute;) e += master.transfer(directions[i], toDistribute - e);
            if (e > 0) storage.modifyEnergyStored(-e);
        }
    }
    ...
    public void mergeGrid(RedstoneEnergyNetwork grid) {
        ...
        regenerating = true;
        for (TileEntityRedNetEnergy cond : grid.conduitSet) addConduit(cond);
        regenerating = r;
