                                                                                                         ...
                                                                                                         // IEnergyInfo
{
    ...
    private void markForRegen() {
        ...
        int c = 0;
        for (int i = 6; i-- > 0;) if ((sideMode[i]) == 9) ++c;
        ...
        if (_grid == null) {
            for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
                ...
                if (BlockPosition.blockExists(this, dir)) {
                    TileEntityRedNetEnergy pipe = BlockPosition
                        .getAdjacentTileEntity(this, dir, TileEntityRedNetEnergy.class);
                    if (pipe != null) {
    ...
    private void addCache(TileEntity tile, int side) {
        ...
        if (receiverCache != null) receiverCache[side] = null;
        if (providerCache != null) providerCache[side] = null;
        if (ic2Cache != null) ic2Cache.erase(side);
        ...
        if (tile instanceof TileEntityRedNetEnergy) {
            ...
            if (((IEnergyConnection) tile).canConnectEnergy(ForgeDirection.VALID_DIRECTIONS[side])) {
                if (tile instanceof IEnergyTransport) {
                    ...
                    InterfaceType type = transport.getTransportState(ForgeDirection.getOrientation(side))
                        .getOpposite();
    ...
    public int getEnergyStored(ForgeDirection from) {
        ...
        if (from == ForgeDirection.UNKNOWN) return 0;
        if ((sideMode[from.ordinal() ^ 1] & 1) != 0 & _grid != null) return _grid.storage.getEnergyStored();
        return 0;
    }
    ...
    public int getMaxEnergyStored(ForgeDirection from) {
        ...
        if (from == ForgeDirection.UNKNOWN) return 0;
        if ((sideMode[from.ordinal() ^ 1] & 1) != 0 & _grid != null) return _grid.storage.getMaxEnergyStored();
        return 0;
    }
    ...
    public void writeToNBT(NBTTagCompound nbt) {
        ...
        if (energyForGrid > 0) nbt.setInteger("Energy", energyForGrid);
        else energyForGrid = 0;
    }
    ...
    void extract(ForgeDirection side, EnergyStorage storage) {
        ...
        if ((sideMode[bSide] & 1) != 0) {
            switch (sideMode[bSide] >> 1) {
                case 1: // IEnergyHandler
                    if (providerCache != null) {
                        ...
                        if (handlerTile != null) {
                            ...
                            if (e > 0) handlerTile.extractEnergy(side, storage.receiveEnergy(e, false), false);
                        }
                    }
                    break;
                ...
                case 3: // IEnergyTile
                    if (ic2Cache != null) ic2Cache.extract(bSide);
                    break;
            ...
            switch (sideMode[bSide] >> 1) {
                case 1: // IEnergyHandler
                    if (receiverCache != null) {
                        ...
                        if (handlerTile != null) return handlerTile.receiveEnergy(side, energy, false);
                    }
                    break;
                case 2: {// IEnergyTransport
                    InterfaceType state = getTransportState(side);
                    if (receiverCache != null && state != InterfaceType.RECEIVE) {
                        IEnergyReceiver handlerTile = receiverCache[bSide];
                        if (handlerTile != null && (state == InterfaceType.SEND
                            || handlerTile.getEnergyStored(side) < _grid.storage.getEnergyStored()))
                            return handlerTile.receiveEnergy(side, energy, false);
                    }
                    break;
                }
                case 3: // IEnergyTile
                    if (ic2Cache != null) return ic2Cache.transmit(energy, side, bSide);
                    break;
                case 4: // TileEntityRednetCable
                case 0: // no mode
                    // no-op
    ...
    private class IC2Cache {
        ...
        void erase(int side) {
            ...
            if (sourceCache != null) sourceCache[side] = null;
            if (sinkCache != null) sinkCache[side] = null;
        }
        ...
        public boolean add(TileEntity tile, int side) {
            ...
            if (tile instanceof IEnergyTile) {
                ...
                if (tile instanceof IEnergySource
                    && ((IEnergySource) tile).emitsEnergyTo(TileEntityRedNetEnergy.this, fSide)) {
                ...
                if (tile instanceof IEnergySink
                    && ((IEnergySink) tile).acceptsEnergyFrom(TileEntityRedNetEnergy.this, fSide)) {
        ...
        void extract(int bSide) {
            ...
            if (sourceCache != null) {
                ...
                if (e > 0) {
                    ...
                    if (e > 0) source.drawEnergy(e / (float) energyPerEU);
                }
            }
            return;
        }
        ...
        int transmit(int energy, ForgeDirection side, int bSide) {
            ...
            if (sinkCache != null) {
                ...
                if (e > 0) {
                    ...
                    e -= (int) Math
                        .ceil(sink.injectEnergy(side, v, getPowerFromTier(getTierFromPower(v))) * energyPerEU);
                    return e;
                }
            }
            return 0;
        ...
        private double getPower(int t) {
            ...
            if (EnergyNet.instance != null) return EnergyNet.instance.getPowerFromTier(t);
            return t * 32;
        }
        ...
        private int getTier(double t) {
            ...
            if (EnergyNet.instance != null) return EnergyNet.instance.getTierFromPower(t);
            return 1;
        }
            ...
            if (MFRUtil.isHoldingUsableTool(player, xCoord, yCoord, zCoord)) {
                if (!player.worldObj.isRemote) {
                    ...
                    if (sideMode[dir] == 8) {
                        deadCache = true;
                        reCache();
                        if (_grid == null) setGrid(new RedstoneEnergyNetwork(this));
                    }
                    RedstoneEnergyNetwork.HANDLER.addConduitForUpdate(this);
                    Packets.sendToAllPlayersWatching(this);
                    worldObj.markBlockForUpdate(xCoord, yCoord, zCoord);
        ...
        for (int i = side.length; i-- > 0;) {
            ...
            l: if (c.isConnected) {
                if (c.isPlate) o += 6;
                    ...
                    list.add((IndexedCuboid6) new IndexedCuboid6(hasTool ? k + 6 * 6 : 1, subSelection[o]).add(offset)); // cable
                                                                                                                         // part
                                                                                                                         // or
                                                                                                                         // connection
                                                                                                                         // point
                ...
                if (forTrace & hasTool & _cableMode[6] != 1 && (c = getConnectionState(side[i], false)).isConnected) { // cable-only
    ...
    public void getTileInfo(List<IChatComponent> info, ForgeDirection side, EntityPlayer player, boolean debug) {
        ...
        if (_grid != null) {/*
            ...
            if (!debug) {
                float sat = 0;
                if (_grid.getNodeCount() != 0) sat = (float) (Math
                    .ceil(_grid.storage.getEnergyStored() / (float) _grid.storage.getMaxEnergyStored() * 1000f) / 10f);
        ...
        if (debug) {
            if (_grid != null) {
                ...
                info.add(
                    text(
                        "Grid Max: " + _grid.storage.getMaxEnergyStored()
                            + ", Grid Cur: "
                            + _grid.storage.getEnergyStored()));
                info.add(
                    text(
                        "Caches: (RF, EU):({" + Arrays
                            .toString(receiverCache) + "," + Arrays.toString(providerCache) + "}, " + ic2Cache + ")"));
            } else {
