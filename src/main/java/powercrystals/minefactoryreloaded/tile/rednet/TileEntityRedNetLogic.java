...
public class TileEntityRedNetLogic extends TileEntityBase implements IRotateableTile, IPortableData {
    ...
    private IRedNetLogicCircuit getNewCircuit(String className) {
        ...
        try {
            return (IRedNetLogicCircuit) Class.forName(className)
                .newInstance();
        } catch (Exception e) {
            e.printStackTrace();
            return new Noop();
    ...
    public void setCircuitFromPacket(NBTTagCompound packet) {
        ...
        try {
            ...
            for (int p = 0; p < len; ++p) pins[p] = new PinMapping(inputs[p << 1], inputs[(p << 1) | 1]);
    ...
    public void sendCircuitDefinition(int circuit) {
        ...
        Packets.sendToAllPlayersInRange(
            worldObj,
            xCoord,
            yCoord,
            zCoord,
    ...
    public void readPortableData(EntityPlayer player, NBTTagCompound tag) {
        ...
        int circuitCount = tag.getTagList("circuits", 10)
            .tagCount();
    ...
    public void writeItemNBT(NBTTagCompound tag) {
        ...
        if (tag.hasKey("circuits", 9)) tag.setByte("p_rot", (byte) worldObj.getBlockMetadata(xCoord, yCoord, zCoord));
        l: for (int v : _upgradeLevel) if (v != 0) {
    ...
    public void writeCricuitsOnly(NBTTagCompound tag, boolean includeNoop) {
        ...
        for (int c = 0; c < _circuits.length; c++) {
            ...
            if (inputPins.tagCount() > 0) circuit.setTag("inputPins", inputPins);
            ...
            if (outputPins.tagCount() > 0) circuit.setTag("outputPins", outputPins);

            NBTTagCompound circuitState = new NBTTagCompound();
            _circuits[c].writeToNBT(circuitState);
            if (!circuitState.hasNoTags()) circuit.setTag("state", circuitState);

            circuits.appendTag(circuit);
        }

        if (circuits.tagCount() > 0) tag.setTag("circuits", circuits);
    }
    ...
    public void readCircuitsOnly(NBTTagCompound tag) {
        ...
        if (circuits != null) {
            ...
            e = _circuits.length;
            for (c = 0; c < e; c++) {
                if (_circuits[c] == null) initCircuit(c, new Noop());
            }
        }
    }
    ...
    public void onDataPacket(NetworkManager net, S35PacketUpdateTileEntity pkt) {
        ...
        switch (pkt.func_148853_f()) {
            case 0:
                _upgradeLevel = pkt.func_148857_g()
                    .getIntArray("upgrades");
                updateUpgradeLevels();
    ...
    private static boolean areEqual(int[] a, int[] b) {
        ...
        for (int i = a.length; i-- > 0;) if (a[i] != b[i]) return false;
