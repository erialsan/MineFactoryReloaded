...
public class FluidTankMulti implements IFluidTank {
    ...
    public void addTank(FluidTankAdv tank) {
        if (tank == null) throw new IllegalArgumentException("null");
        for (int i = length; i-- > 0;) if (tanks[i] == tank) return;
    ...
    public void empty() {
        for (int i = length; i-- > 0;) tanks[i] = null;
        length = 0;
        index = 0;
        fluid = null;
    }
