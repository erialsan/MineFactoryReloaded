...
public class GridTickHandler<G extends IGrid, N extends INode> implements IGridController {
    ...
    public static final GridTickHandler<RedstoneEnergyNetwork, TileEntityRedNetEnergy> energy = new GridTickHandler<RedstoneEnergyNetwork, TileEntityRedNetEnergy>(
        "Energy");
    public static final GridTickHandler<RedstoneNetwork, TileEntityRedNetCable> redstone = new GridTickHandler<RedstoneNetwork, TileEntityRedNetCable>(
        "Redstone");
    public static final GridTickHandler<FluidNetwork, TileEntityPlasticPipe> fluid = new GridTickHandler<FluidNetwork, TileEntityPlasticPipe>(
        "Fluid");
    ...
    public void tick(ServerTickEvent evt) {
        ...
        if (evt.phase == Phase.START) tickStart();
        else tickEnd();
    }
    ...
    public void tickStart() {
        ...
        if (!tickingGridsToRegenerate.isEmpty()) synchronized (tickingGridsToRegenerate) {
            for (G grid : tickingGridsToRegenerate) grid.markSweep();
        ...
        if (!conduitToUpd.isEmpty()) synchronized (conduitToUpd) {
        ...
        if (!conduit.isEmpty()) {
            ...
            try {
                ...
                while (iter.hasNext()) {
                    cond = iter.next();
                    if (!cond.isNotValid()) cond.updateInternalTypes(this);
                }
                conduit.clear();
            } catch (Throwable throwable) {
                RuntimeException error = new RuntimeException("Crashing on conduit " + cond, throwable);
        ...
        if (!tickingGrids.isEmpty()) for (G grid : tickingGrids) grid.doGridPreUpdate();
        // }
    }
    ...
    public void tickEnd() {
        ...
        if (!tickingGridsToRemove.isEmpty()) synchronized (tickingGridsToRemove) {
            tickingGrids.removeAll(tickingGridsToRemove);
            tickingGridsToRemove.clear();
        }

        if (!tickingGridsToAdd.isEmpty()) synchronized (tickingGridsToAdd) {
        ...
        if (!tickingGrids.isEmpty()) for (G grid : tickingGrids) grid.doGridUpdate();
        // }

        // { Initial update tick for conduits added to the world
        if (!conduitToAdd.isEmpty()) synchronized (conduitToAdd) {
        ...
        if (!conduit.isEmpty()) {
            ...
            try {
                ...
                while (iter.hasNext()) {
                    cond = iter.next();
                    if (!cond.isNotValid()) cond.firstTick(this);
                }
                conduit.clear();
            } catch (Throwable throwable) {
                RuntimeException error = new RuntimeException("Crashing on conduit " + cond, throwable);
