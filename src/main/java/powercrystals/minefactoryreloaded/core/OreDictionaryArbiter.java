 ...
 * 
 * The vast majority of this functionality is safely exposed through {@link ItemHelper} via a proxy. If you use the
 * fancy functions here, READ how they work.
...
public class OreDictionaryArbiter {
    ...
    public static void bake() {
        ...
        for (Entry<ItemWrapper, ArrayList<Integer>> a : ids.entrySet()) stackIDs.put(a.getKey(), a.getValue());

        for (Entry<ItemWrapper, ArrayList<String>> a : names.entrySet()) stackNames.put(a.getKey(), a.getValue());
    }
    ...
    public static void registerOreDictionaryEntry(ItemStack stack, String name) {
        ...
        stackIDs.get(item)
            .add(OreDictionary.getOreID(name));
        stackNames.get(item)
            .add(name);
    }
     ...
     * 
     * Forge's Ore Dictionary has an O(N) copy here because it assumes all modders are stupid. But you're not stupid,
     * right?
     * 
     * DO NOT MODIFY THESE.
     */
