...
public class ContainerSteamBoiler extends ContainerFactoryInventory {
    ...
    protected void addSlots() {
        for (int i = 0; i < 4; ++i) addSlotToContainer(new Slot(_te, i, 8 + i * 18, 23));
    }
    ...
    public void detectAndSendChanges() {
        ...
        for (int i = 0; i < crafters.size(); i++) {
            ((ICrafting) crafters.get(i))
                .sendProgressBarUpdate(this, 0, ((TileEntitySteamBoiler) _te).getWorkDone() & 65535);
            ((ICrafting) crafters.get(i))
                .sendProgressBarUpdate(this, 1, ((TileEntitySteamBoiler) _te).getWorkDone() >> 16);
            int temp = (int) (((TileEntitySteamBoiler) _te).getTemp() * 10);
            ((ICrafting) crafters.get(i)).sendProgressBarUpdate(this, 2, temp);
            ((ICrafting) crafters.get(i))
                .sendProgressBarUpdate(this, 3, ((TileEntitySteamBoiler) _te).getWorkMax() & 65535);
            ((ICrafting) crafters.get(i))
                .sendProgressBarUpdate(this, 4, ((TileEntitySteamBoiler) _te).getWorkMax() >> 16);
        }
    }
