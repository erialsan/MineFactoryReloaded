...
public class GuiHarvester extends GuiUpgradeable {
    ...
    protected void actionPerformed(GuiButton button) {
        ...
        if (button.id == 1) {
            Packets.sendToServer(Packets.HarvesterButton, _tileEntity, "silkTouch", getNewSettingValue("silkTouch"));
        } else if (button.id == 2) {
            Packets.sendToServer(
                Packets.HarvesterButton,
                _tileEntity,
                "harvestSmallMushrooms",
                getNewSettingValue("harvestSmallMushrooms"));
        } else if (button.id == 3) {
            // PacketDispatcher.sendPacketToServer(PacketWrapper.createPacket(MineFactoryReloadedCore.modNetworkChannel,
            // Packets.HarvesterButton,
    ...
    private String getSettingText(String setting) {
        ...
        return _harvester.getSettings()
            .get(setting) == Boolean.TRUE ? "Yes" : "No";
    }

    private Boolean getNewSettingValue(String setting) {

        return _harvester.getSettings()
            .get(setting) == Boolean.TRUE ? false : true;
    }
