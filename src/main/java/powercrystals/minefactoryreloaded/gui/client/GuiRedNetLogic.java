...
public class GuiRedNetLogic extends GuiBase {
    ...
    private class CircuitComparator implements Comparator<IRedNetLogicCircuit> {
        ...
        public int compare(IRedNetLogicCircuit arg0, IRedNetLogicCircuit arg1) {
            ...
            if (a != b) return a ? -1 : 1;
            return MFRUtil.localize(arg0.getUnlocalizedName())
                .compareTo(MFRUtil.localize(arg1.getUnlocalizedName()));
        }
    }
    ...
    public void initGui() {
        ...
        uFontRenderer = new FontRenderer(
            mc.gameSettings,
        ...
        _reinitConfirm = new ElementButtonManaged(this, 318, 205, 60, 20, "Confirm") {
            ...
            public void onClick() {
                ...
                Packets
                    .sendToServer(Packets.LogicReinitialize, _logic, Minecraft.getMinecraft().thePlayer.getEntityId());
                _reinitCountdown = 0;
                _listNeedsUpdated = true;
            }
        };
        ...
        int rotation = _logic.getWorldObj()
            .getBlockMetadata(_logic.xCoord, _logic.yCoord, _logic.zCoord);

        for (int i = 0; i < _inputIOPinButtons.length; i++) {
            _inputIOBufferButtons[i] = new ButtonLogicBufferSelect(
                this,
                25,
                16 + i * pinOffset,
                i,
                LogicButtonType.Input,
                rotation);
            _inputIOPinButtons[i] = new ButtonLogicPinSelect(this, 54, 16 + i * pinOffset, i, LogicButtonType.Input);

            _outputIOBufferButtons[i] = new ButtonLogicBufferSelect(
                this,
                254,
                16 + i * pinOffset,
                i,
                LogicButtonType.Output,
                rotation);
        ...
        if (c != null && c != _logic.getCircuit(_selectedCircuit)) {
            if (_listNeedsUpdated) {
                for (int i = 0; i < _circuitList.getElementCount(); i++)
                    if (((IRedNetLogicCircuit) _circuitList.getElement(i)
                        .getValue()).getClass() == _logic.getCircuit(_selectedCircuit)
                            .getClass()) {
            ...
            for (int i = 0; i < _inputIOPinButtons.length; i++) {
                if (i < _logic.getCircuit(_selectedCircuit)
                    .getInputCount()) {
    ...
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        ...
        if (name == null) name = MFRUtil.localize("tile.mfr.rednet.logic");
        ...
        for (int i = 0; i < _inputIOPinButtons.length; i++) {
            if (i < _logic.getCircuit(_selectedCircuit)
                .getInputCount()) {
                rFontRenderer.drawString(
                    _logic.getCircuit(_selectedCircuit)
                        .getInputPinLabel(i),
                    5,
                    20 + i * pinOffset,
                    4210752);
    ...
    private void requestCircuit() {
        ...
        Packets.sendToServer(Packets.CircuitDefinition, _logic, _selectedCircuit);
    }

    public void setInputPinMapping(int index, int buffer, int pin) {

        Packets.sendToServer(Packets.LogicSetPin, _logic, (byte) 0, _selectedCircuit, index, buffer, pin);
    }

    public void setOutputPinMapping(int index, int buffer, int pin) {

        Packets.sendToServer(Packets.LogicSetPin, _logic, (byte) 1, _selectedCircuit, index, buffer, pin);
    }
