...
public class Machine {
    ...
    public static Machine AutoEnchanter = new Machine(
        0,
        14,
        "AutoEnchanter",
        TileEntityAutoEnchanter.class,
        160,
        16000);
        ...
        public void addInformation(ItemStack stack, EntityPlayer player, List<String> info, boolean adv) {
            ...
            if (c != null && c.hasKey("storedStack")) {
                ...
                if (storedItem != null & storedQuantity > 0) {
                    info.add(
                        String.format(
                            MFRUtil.localize("tip.info.mfr.dsu.contains", true),
                            storedQuantity + " "
                                + storedItem.getDisplayName()
                                + (adv
                                    ? " (" + Item.itemRegistry.getIDForObject(storedItem.getItem())
                                        + ":"
    ...
    public static Machine LavaFabricator = new Machine(
        1,
        5,
        "LavaFabricator",
        TileEntityLavaFabricator.class,
        800,
        16000);
    ...
    public static Machine BioFuelGenerator = new Machine(
        1,
        11,
        "BioFuelGenerator",
        TileEntityBioFuelGenerator.class,
        160,
        10000);
    public static Machine AutoDisenchanter = new Machine(
        1,
        12,
        "AutoDisenchanter",
        TileEntityAutoDisenchanter.class,
        320,
        16000) {
    ...
    public static Machine Slaughterhouse = new Machine(
        1,
        13,
        "Slaughterhouse",
        TileEntitySlaughterhouse.class,
        1000,
        16000);
    public static Machine MeatPacker = new Machine(1, 14, "MeatPacker", TileEntityMeatPacker.class, 20, 16000);
    public static Machine EnchantmentRouter = new Machine(
        1,
        15,
        "EnchantmentRouter",
        TileEntityEnchantmentRouter.class);

    public static Machine LaserDrill = new Machine(2, 0, "LaserDrill", TileEntityLaserDrill.class);
    public static Machine LaserDrillPrecharger = new Machine(
        2,
        1,
        "LaserDrillPrecharger",
        TileEntityLaserDrillPrecharger.class,
        5000,
        96000);
    ...
    public static Machine ChunkLoader = new Machine(
        2,
        10,
        "ChunkLoader",
        ...
        public void load(Configuration c) {
            ...
            if (!MFRConfig.enableConfigurableCLEnergy.getBoolean(false)) _activationEnergy = 0;
            super.load(c);
        }
    };
    ...
    protected Machine(int blockIndex, int meta, String name, Class<? extends TileEntityFactory> tileEntityClass) {
    ...
    protected Machine(int blockIndex, int meta, String name, Class<? extends TileEntityFactory> tileEntityClass,
        ...
        if (_machineMappings.get(_machineIndex) != null) {
            throw new IllegalArgumentException(
                "Machine with index " + blockIndex + " and meta " + meta + " already exists.");
        }
    ...
    public boolean hasTooltip(ItemStack stack) {
        ...
        if (stack.stackTagCompound != null)
            if (_energyStoredMax > 0 && stack.stackTagCompound.hasKey("energyStored")) return true;
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List<String> info, boolean adv) {
        ...
        if (_activationEnergy > 0) {
            if (_generator) {
                info.add(
                    MFRUtil.localize("info.cofh.energyProduce", true) + ": "
                        + GREEN
                        + _activationEnergy
                        + " RF/t"
                        + RESET);
                ...
                MFRUtil.localize("info.cofh.energyConsume", true) + ": " + RED + _activationEnergy + " RF/Wk" + RESET);
        }
    ...
    public void load(Configuration c) {
        ...
        if (_activationEnergy > 0) {
            ...
            if (_generator) comment = "The amount of energy generated by this machine in one tick";
            if (_useDaRF) _activationEnergy = c
                .get(
                    "Machine." + _name,
                    "ActivationCostDaRF",
                    getActivationEnergyDaRF(),
                    comment + ", in units of 10 RF (i.e., 2 DaRF = 20 RF)")
                .setRequiresMcRestart(true)
                .getInt() * 10;
            else _activationEnergy = c
                .get("Machine." + _name, "ActivationCostRF", getActivationEnergy(), comment + ", in units of **1** RF")
                .setRequiresMcRestart(true)
                .getInt();
        }

        MFRThings.machineBlocks.get(_blockIndex)
            .setHarvestLevel("pickaxe", 0, _meta);
    ...
    protected String loadIcon(Side side, boolean active) {
        ...
        if (CoFHProps.enableColorBlindTextures) {
            final String cb = ".cb";
            if (RegistryUtils.blockTextureExists(t = base + name + a + cb)) return t;
            else if (side.hasAlt && RegistryUtils.blockTextureExists(t = base + name + side.getAlt(active) + cb))
                return t;
            else if (RegistryUtils.blockTextureExists(t = base + name + side.name + cb)) return t;
            else if (side.hasAlt && RegistryUtils.blockTextureExists(t = base + name + side.alt + cb)) return t;
        }
        if (RegistryUtils.blockTextureExists(t = base + name + a)) return t;
        else if (side.hasAlt && RegistryUtils.blockTextureExists(t = base + name + side.getAlt(active))) return t;
        else if (RegistryUtils.blockTextureExists(t = base + name + side.name)) return t;
        else if (side.hasAlt && RegistryUtils.blockTextureExists(t = base + name + side.alt)) return t;
        else if (RegistryUtils.blockTextureExists(t = base + "tile.mfr.machine.0." + a)) return t;
        else if (side.hasAlt
            && RegistryUtils.blockTextureExists(t = base + "tile.mfr.machine.0." + side.getAlt(active))) return t;
        else if (RegistryUtils.blockTextureExists(t = base + "tile.mfr.machine.0." + side.name)) return t;
        else return base + "tile.mfr.machine.0." + side.alt;
    }
