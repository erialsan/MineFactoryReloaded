...
public class ItemBlockFactoryMachine extends ItemBlockFactory implements IEnergyContainerItem {
    ...
    public ItemBlockFactoryMachine(net.minecraft.block.Block blockId) {
        ...
        for (int i = 0; i <= highestMeta; i++) {
            names[i] = Machine.getMachineFromIndex(_machineBlockIndex, i)
                .getInternalName();
        }
        setNames(names);
        for (int i = 0; i <= highestMeta; i++) {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List info, boolean adv) {
        ...
        if (!machine.hasTooltip(stack)) return;
    ...
    public int receiveEnergy(ItemStack container, int maxReceive, boolean simulate) {
        ...
        if (!simulate & received > 0) setEnergy(container, newEnergy);
        return received;
    }
    ...
    public int extractEnergy(ItemStack container, int maxExtract, boolean simulate) {
        ...
        if (!simulate & removed > 0) setEnergy(container, newEnergy);
        return removed;
    }
    ...
    public int getEnergyStored(ItemStack container) {
        ...
        if (container.hasTagCompound()) return container.getTagCompound()
            .getInteger("energyStored");
        return 0;
    }
