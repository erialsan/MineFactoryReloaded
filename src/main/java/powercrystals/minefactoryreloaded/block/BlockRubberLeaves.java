...
public class BlockRubberLeaves extends BlockLeaves implements IRedNetNoConnection {
    ...
    public int colorMultiplier(IBlockAccess iba, int x, int y, int z) {
        ...
        for (int l1 = -1; l1 <= 1; ++l1) for (int i2 = -1; i2 <= 1; ++i2) {
            int j2 = iba.getBiomeGenForCoords(x + i2, z + l1)
        ...
        g = (g / 9 & 255);
        b = (b / 9 & 255);
        if (meta == 1) return (r / 4 << 16 | g / 4 << 8 | b / 4) + 0xc0c0c0;
    ...
    public ArrayList<ItemStack> getDrops(World world, int x, int y, int z, int meta, int fortune) {
        ...
        if (fortune > 0) chance = Math.max(chance - (2 << fortune), 10);

        if (world.rand.nextInt(chance) == 0) ret.add(
            new ItemStack(getItemDropped(meta & 3, world.rand, fortune), 1, world.rand.nextInt(50000) == 0 ? 2 : 0));
        ...
        if (meta == 0 & ((l & 4) == 0)) {
            ...
            if (decay) {
                ...
                if (!world.getBlock(x, y, z)
                    .equals(this)) dropBlockAsItem(world, x, y, z, l, 0);
                return;
            }
            int chance = 15;
            ...
            if (b != null) {
                ...
                decay |= (t = temp > 1.8f);
                if (t) chance -= 5;
                if (rain >= 0.4f & temp <= 1.4f) chance += 7;
                else if (temp < 0.8f) chance += 3;
            }
    ...
    public void breakBlock(World world, int x, int y, int z, Block id, int meta) {
        if (updating.get() != null) {
            ...
            if (decay && world.rand.nextInt(chance) == 0) world.setBlock(x, y, z, id, meta | 1, 2);
        }
    }
    ...
    public boolean shouldSideBeRendered(IBlockAccess iba, int x, int y, int z, int side) {
        ...
        if (cube && iba.getBlock(x, y, z) == this) return false;
